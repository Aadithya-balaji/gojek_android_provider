...
class FilterServiceListAdapter(val orderFragmentViewModel: DashBoardViewModel,
                               val filterServiceListName: List<ConfigService>,
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): MyViewHolder {
        ...
        val inflate = DataBindingUtil.inflate<FilterserviceRowitemBinding>(LayoutInflater.from(parent.context),
    ...
    override fun onBindViewHolder(holder: MyViewHolder, position: Int) {
        if (selectedPosition == position && selectedPosition != -1)
            ...
            holder.filterserviceRowitemBinding.filterserviceNameTv.background =
                    ContextCompat.getDrawable(context, R.drawable.custom_roundcorner_selectedorder)
        else holder.filterserviceRowitemBinding.filterserviceNameTv.background =
                ContextCompat.getDrawable(context, R.drawable.custom_roundcorner_unselectedorder)
        holder.filterserviceRowitemBinding.filterserviceNameTv.text = (filterServiceListName[position].admin_service_name).toLowerCase().capitalize()
        holder.filterserviceRowitemBinding.itemClickListener = object : CustomClickListner {
            override fun onListClickListner() {
                ...
                serviceTypeListener.getServiceType(filterServiceListName[position].admin_service_name.toLowerCase(), filterServiceListName.get(position).id)
