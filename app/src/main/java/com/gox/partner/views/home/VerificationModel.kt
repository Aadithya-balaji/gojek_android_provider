...
class VerificationModel {
    ...
    fun isNeedToShowPendingDialog(): Boolean {
        if (isService <= 0 || isDocument <= 0 || isBankDetail <= 0  || isProfile <= 0) return true
    ...
    fun getDialogType(): String {
        if (isService <= 0 || isDocument <= 0 || isBankDetail <= 0 || isProfile <= 0) return Constants.ProviderStatus.PENDING
